...
DROP TABLE IF EXISTS elk_user_log;
...
CREATE TABLE secret_friends (
	...
	CONSTRAINT fk_secret_friends_host  FOREIGN KEY(host_id) REFERENCES account_privacy(account_id) ON DELETE CASCADE,
	CONSTRAINT fk_secret_friends_guest  FOREIGN KEY(guest_id) REFERENCES account_privacy(account_id) ON DELETE CASCADE
...
CREATE TABLE secret_post (
    ...
    secret_writer_id			int				NOT NULL							 COMMENT 'Auto increament',
	...
	CONSTRAINT fk_secret_post_secret_writer_id  FOREIGN KEY(secret_writer_id) REFERENCES account_privacy(account_id) ON DELETE CASCADE
...
CREATE TABLE elk_user_log (
	elk_id	                int	            AUTO_INCREMENT	    PRIMARY KEY	    COMMENT 'Auto increament',
	elk_login_time		DATETIME 			not null 							COMMENT '유저의 접속 시간'
);
